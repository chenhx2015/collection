...
// 因为是最近，所以遍历顺序一定是后序遍历，利用递归回溯的特点，
// 遇到的第一个满足条件一的节点一定就是最近公共祖先，直接返回，
// 后续检测到返回的是节点而不是null后，继续将收到的返回值返回就可。
...
// 分析：
// 记录当前节点的左子树，右子树有没有找到目标节点和当前节点是否是目标节点，
// 因为题目中给出一个节点也可以是他自己的祖先节点，那么就有如下四种情况：
